...
describe("count", () => {
  ...
  it("counts the number of items matching predicate", () => {
    const array = [1, 1, 2, 2, 3, 4]
    expect(count(array, n => n > 2)).toBe(2)
    expect(count(array, n => n > 3)).toBe(1)
    expect(count(array, n => n > 4)).toBe(0)
  })
})
...
describe("remove", () => {
  ...
  it("removes items matching predicate", () => {
    const array = [1, 1, 2, 2, 3, 4]
    expect(remove(array, n => n > 2)).toStrictEqual([1, 1, 2, 2])
    expect(remove(array, n => n > 3)).toStrictEqual([1, 1, 2, 2, 3])
    expect(remove(array, n => n > 4)).toStrictEqual([1, 1, 2, 2, 3, 4])
  })
})
